...
namespace Plantjes.ViewModels
{
        ...
        foreach (FenoKleur item in searchService.GetList<FenoKleur>())
        {
            yield return new MenuItem()
            {
                ...
                Header = new Rectangle()
                {
                    Width = 20,
                    Height = 20,
                    Fill = (SolidColorBrush)new BrushConverter().ConvertFromString("#" +
                        Convert.ToHexString(item.HexWaarde)),
                },
    ...
    private void AddPlant(object parameters)
    {
        ...
        if (MBezonning.Any(mi => mi.IsChecked) || MGrondsoort.Any(mi => mi.IsChecked) ||
            MVoedingsbehoefte.Any(mi => mi.IsChecked))
        {
            ...
            DaoAbiotiek.AddAbiotiek(plant, bezonning, grondsoort,
                string.IsNullOrEmpty(items[0] as string) ? null : items[0] as string, voedingsBehoefte);
        }
        ...
        foreach (Beheersdaad beheersdaad in IctrlBeheersdaad)
        {
            if (!string.IsNullOrEmpty(beheersdaad.BeheersdaadText))
                DaoBeheersdaden.AddBeheersdaden(plant, beheersdaad.BeheersdaadText,
                    beheersdaad.Months.Select(mi => mi.IsChecked).ToList());
        }
        ...
        if (MStrategie.Any(mi => mi.IsChecked))
        {
            ...
            DaoCommensalisme.AddCommensalisme(plant,
                string.IsNullOrEmpty(items[1] as string) ? null : items[1] as string, strategie);
        }
    ...
    public IEnumerable<TfgsvFamilie> CmbFamilies
    {
        get => SelectedType == null
            ...
            : searchService.GetListWhere<TfgsvFamilie>(f => f.TypeTypeid == SelectedType.Planttypeid)
                .OrderBy(f => f.Familienaam);
    }
    ...
    public IEnumerable<TfgsvGeslacht> CmbGeslacht
    {
        get => SelectedFamilie == null
            ...
            : searchService.GetListWhere<TfgsvGeslacht>(g => g.FamilieFamileId == SelectedFamilie.FamileId)
                .OrderBy(g => g.Geslachtnaam);
    }
    ...
    public IEnumerable<TfgsvSoort> CmbSoort
    {
        get => SelectedGeslacht == null
            ...
            : searchService.GetListWhere<TfgsvSoort>(s => s.GeslachtGeslachtId == SelectedGeslacht.GeslachtId)
                .OrderBy(s => s.Soortnaam);
    }
    ...
    public IEnumerable<TfgsvVariant> CmbVariant
    {
        get => SelectedSoort == null
            ...
            : searchService.GetListWhere<TfgsvVariant>(v => v.SoortSoortid == SelectedSoort.Soortid)
                .OrderBy(v => v.Variantnaam);
    }
    ...
    {
        get => searchService.GetListWhere<FenotypeMulti>(f =>
                f.Eigenschap.ToLower() == "bladhoogtemax" && f.Maand.ToLower() == SelectedFenotypeMaand.ToLower())
            .Select(f => f.Waarde);
    }
    ...
    {
        get => searchService.GetListWhere<FenotypeMulti>(f =>
                f.Eigenschap.ToLower() == "bloeihoogtemin" && f.Maand.ToLower() == SelectedFenotypeMaand.ToLower())
            .Select(f => f.Waarde);
    }
    ...
    {
        get => searchService.GetListWhere<FenotypeMulti>(f =>
                f.Eigenschap.ToLower() == "bloeihoogtemax" && f.Maand.ToLower() == SelectedFenotypeMaand.ToLower())
            .Select(f => f.Waarde);
    }
